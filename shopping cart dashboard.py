...
def create_sidebar(root):
    ...
    chatbot_button = ttk.Button(sidebar, text="Chatbot", command=open_chatbot_popup)
    chatbot_button.pack(pady=15)
def open_chatbot_popup():
    chatbot_popup = tk.Toplevel()
    chatbot_popup.title("Chatbot")
    
    chat_frame = ttk.Frame(chatbot_popup)
    chat_frame.pack(padx=20, pady=20)

    conversation_display = Listbox(chat_frame, height=20, width=50)
    conversation_display.pack(padx=10, pady=10)

    entry_field_chat = Entry(chat_frame, width=50)
    entry_field_chat.pack(padx=10, pady=(0, 10))

    def send_message(event=None):
        message = entry_field_chat.get()
        entry_field_chat.delete(0, END)
        conversation_display.insert(END, "You: " + message)
        bot_response = get_bot_response(message)
        conversation_display.insert(END, "Bot: " + bot_response)

    entry_field_chat.bind("<Return>", send_message)
    entry_field_chat.focus()

def get_bot_response(user_message):
    # This is a placeholder function for the actual chatbot logic
    # You can replace this with your actual chatbot implementation
    # For example, you can use a third-party API to get responses
